...
public class GuiGrabMouse extends ScreenObject {
	...
	public GuiGrabMouse(ScreenObject parent, Vector2f position, String prefix, int value, GuiAlign align) {
	...
	public void updateObject() {
		...
		if (selected) {
			...
			for (int i = 0; i < Constants.GLFW_MOUSE_BUTTON_LAST; i++) {
				if (FlounderMouse.get().getMouse(i)) {
					if (i == 0) {
						if (FlounderGuis.get().getSelector().wasLeftClick() && FlounderGuis.get().getSelector().isSelected(text)) {
							key = i;
						}
					} else {
						key = i;
